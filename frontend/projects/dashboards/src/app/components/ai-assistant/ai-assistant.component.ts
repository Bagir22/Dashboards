...
export class AiAssistantComponent implements OnChanges, OnInit {
  ...
  protected async copyToClipboard(text: string, messageIndex?: number): Promise<void> {
    try {
  ...
  private addWelcomeMessage(): void {
    this.messages.push({
      text: 'Здравствуйте! Я ИИ-ассистент. Что вы хотите проанализировать?',
      isUser: false,
      timestamp: new Date(),
      isNotification: true,
    });
  ...
  protected async executeAction(entity: TabInfo | TabCard, index?: number): Promise<void> {
    if (this.shouldBlockAction()) return;

    const cardsLength = (<TabInfo & TabCard>entity).cards?.length;

    if (cardsLength && cardsLength > 1 &&
      this.groupedCardsbyTabs?.length && this.groupedCardsbyTabs?.length > 1 &&
    ...
    try {
      const cardIds = cardsLength
        ? (<TabInfo>entity).cards.map(card => card.id)
        : [(<TabCard>entity).id];
      await this.streamResponse(cardIds);
    } catch (error) {
  ...
  private async streamResponse(cardIds: number[]): Promise<void> {
    const response = cardIds.length > 1
      ? await this.makeCardsApiRequest(cardIds)
      : await this.makeCardApiRequest(cardIds[0]);
